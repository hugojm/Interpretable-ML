...
PERTURB_LIME_MODE_STD = 0
PERTURB_LIME_MODE_SBO = 1
PERTURB_LIME_MODE_GHO = 2

PERTURB_SHAP_MODE_STD = 0
PERTURB_SHAP_MODE_BIRCH = 1
...
class Adversarial_Lime_Model(Adversarial_Model):
	...
	def train(self, X, y, feature_names, perturbation_multiplier=30, categorical_features=[], rf_estimators=100, estimator=None, perturbation_mode=PERTURB_LIME_MODE_STD):
		...
		if perturbation_mode == PERTURB_LIME_MODE_STD:
		...
		elif perturbation_mode == PERTURB_LIME_MODE_SBO:
			perturbed_xtrain = soft_brownian_offset(X, d_min=.35, d_off=.24, n_samples=perturbation_multiplier*X.shape[0], softness=0)
			all_x = np.vstack((X, perturbed_xtrain))
			all_x = np.vstack(all_x)
			all_y = np.ones(X.shape[0])
			all_y = np.append(all_y, np.zeros(perturbation_multiplier*X.shape[0]))
		elif perturbation_mode == PERTURB_LIME_MODE_GHO:
			perturbed_xtrain = (gaussian_hyperspheric_offset(n_samples=perturbation_multiplier*X.shape[0], mu=2, std=.3, n_dim=X.shape[1]) + X.mean()) * X.std()
			all_x = np.vstack((X, perturbed_xtrain))
			all_x = np.vstack(all_x)
			all_y = np.ones(X.shape[0])
			all_y = np.append(all_y, np.zeros(perturbation_multiplier*X.shape[0]))
		#print([feature_names.index(c) for c in feature_names], categorical_features)
...
class Adversarial_Kernel_SHAP_Model(Adversarial_Model):
	...
	def train(self, X, y, feature_names, background_distribution=None, perturbation_multiplier=10, n_samples=2e4, rf_estimators=100, n_kmeans=10, estimator=None, perturbation_mode=PERTURB_SHAP_MODE_STD):
		...
		if background_distribution is None:
			if perturbation_mode == PERTURB_SHAP_MODE_STD:
				background_distribution = shap.kmeans(X,n_kmeans).data
			elif perturbation_mode == PERTURB_SHAP_MODE_BIRCH:
				birch = Birch().fit(X)
				background_distribution = birch.subcluster_centers_
